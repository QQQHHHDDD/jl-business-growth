...
func TestLoadUsesProcessEnvironment(t *testing.T) {
	...
	t.Setenv("MAX_REQUEST_BODY_MB", "60")
...
func TestReleaseRepositoryIsFixed(t *testing.T) {
	config := Config{AppEnv: test, DatabaseURL: "postgres://example", ReleaseRepository: "attacker/example"}
...
func TestProductionRequiresSecureCookieAndSessionSecret(t *testing.T) {
	config := Config{AppEnv: production, DatabaseURL: "postgres://example"}
...
func TestNonProductionRejectsSecureCookie(t *testing.T) {
	for _, environment := range []string{development, test} {
		config := Config{AppEnv: environment, DatabaseURL: "postgres://example", CookieSecure: true}
		if err := config.Validate(); err == nil {
...
func TestProductionReleaseRuntimeRootMatchesSystemdPath(t *testing.T) {
	config := Config{AppEnv: production, DatabaseURL: "postgres://example", CookieSecure: true, SessionSecret: "secret", ReleaseRepository: "QQQHHHDDD/jl-business-growth", ReleaseRuntimeRoot: "/tmp/release-updater"}
	if err := config.Validate(); err == nil {
