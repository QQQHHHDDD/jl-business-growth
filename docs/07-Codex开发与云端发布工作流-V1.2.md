# JL团队生意成长管理系统——Codex 开发与云端发布工作流 V1.2
...
## 9. Phase 0–6 完成后的固定流程

Phase 0–6 主体代码完成后，不直接部署生产。进入：
...
当前已经完成 Phase 0–6 实现审计、V1 收口和第 7 阶段综合测试。后续开发以根目录 `AGENTS.md`、`docs/00-文档索引.md` 和当前活动实施计划为准；未经正式发布验收，不得把当前分支视为正式 v1.0.0。
...
不得借收口阶段新增新的产品范围。收口验收通过后正式功能冻结，再进入综合测试。
