...
describe("App", () => {
  ...
  it("switches between login and registration shells", async () => {
    ...
    fireEvent.click(screen.getByRole("link", { name: "首次使用？注册普通用户" }));
    expect(await screen.findByRole("heading", { name: "注册普通用户" })).toBeInTheDocument();
  ...
  it("returns to the login shell after changing the password", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("heading", { name: "账号设置" })).toBeInTheDocument());
  ...
  it("redirects a normal user away from administrator routes", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn((input: RequestInfo | URL) => {
      const path = String(input);
      if (path.endsWith("/api/auth/me")) return Promise.resolve(new Response(JSON.stringify(authResponse), { status: 200 }));
      return Promise.resolve(new Response(JSON.stringify({ data: { status: "ok", checks: {} }, request_id: "request-1" }), { status: 200 }));
    }));
    ...
    expect(screen.queryByRole("navigation", { name: "管理员导航" })).not.toBeInTheDocument();
  });

  it("redirects an ordinary administrator away from the super administrator route", async () => {
    const account = { id: "00000000-0000-0000-0000-000000000002", username: "admin", role: "ADMIN", status: "ACTIVE", timezone: "Asia/Shanghai", created_at: "2026-01-01T00:00:00Z", last_login_at: null };
    const authResponse = { data: { account, accounts: [{ ...account, active: true }], csrf_token: "csrf-token" }, request_id: "request-2" };
    ...
    await waitFor(() => expect(screen.getByRole("heading", { name: "管理员工作台" })).toBeInTheDocument());
    expect(window.location.pathname).toBe("/admin");
    expect(screen.queryByRole("link", { name: "管理员管理" })).not.toBeInTheDocument();
  });

  it("redirects an administrator away from ordinary user business routes", async () => {
    const account = { id: "00000000-0000-0000-0000-000000000003", username: "admin", role: "ADMIN", status: "ACTIVE", timezone: "Asia/Shanghai", created_at: "2026-01-01T00:00:00Z", last_login_at: null };
    const authResponse = { data: { account, accounts: [{ ...account, active: true }], csrf_token: "csrf-token" }, request_id: "request-2" };
