...
export function DataPage({ authResponse }: { authResponse: AuthResponse }) {
  ...
  const selectType = (value: ImportType) => { setType(value); setFile(null); setJob(null); setNotice(""); setError(""); setStep(2); };
  ...
  return <div className="space-y-6">
    ...
    <ol className="grid gap-2 sm:grid-cols-5" aria-label="导入步骤">{steps.map((label, index) => { const number = index + 1; const active = number === step; const complete = number < step; return <li key={label} aria-current={active ? "step" : undefined} className={`flex items-center gap-3 rounded-lg border px-3 py-3 text-sm ${active ? "border-teal-500 bg-teal-50 text-teal-900" : complete ? "border-teal-200 bg-white text-teal-700" : "border-slate-200 bg-white text-slate-400"}`}><span className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-bold ${active ? "bg-teal-700 text-white" : complete ? "bg-teal-100 text-teal-800" : "bg-slate-100"}`}>{complete ? <Check size={14} /> : number}</span><span className="font-semibold">{label}</span></li>; })}</ol>
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
      <Panel title={`模板导入 · ${steps[step - 1]}`} description={typeLabel ? `当前类型：${typeLabel}` : "请选择本次导入的数据类型。"}>
        {step === 1 && <section aria-labelledby="import-step-1"><StepTitle number="1" title="选择类型" /><div className="mt-4 grid gap-3 sm:grid-cols-2">{importTypes.map((item) => <button key={item.value} type="button" className="rounded-lg border border-slate-200 p-4 text-left text-sm font-bold text-slate-800 transition hover:border-teal-400 hover:bg-teal-50 focus:outline-none focus:ring-2 focus:ring-teal-500" onClick={() => selectType(item.value)}>{item.label}</button>)}</div></section>}
        ...
        {step === 3 && type && <section aria-labelledby="import-step-3"><StepTitle number="3" title="上传文件" /><p className="mt-3 text-sm text-slate-600">当前类型：{typeLabel}</p><label className="mt-4 block rounded-lg border border-dashed border-slate-300 bg-slate-50 p-5 text-center"><FileSpreadsheet className="mx-auto text-teal-700" size={28} /><span className="mt-2 block text-sm font-semibold text-slate-800">选择 XLSX 文件</span><input aria-label="选择 XLSX 文件" type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" className="mt-3 block w-full text-sm" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /></label><div className="mt-5 flex items-center justify-between gap-3"><Button variant="secondary" onClick={() => setStep(2)}><ChevronLeft size={15} />上一步</Button><Button onClick={() => uploadMutation.mutate()} loading={uploadMutation.isPending} disabled={!file}><FileUp size={15} />{uploadMutation.isPending ? "处理中..." : "上传并校验"}</Button></div></section>}
