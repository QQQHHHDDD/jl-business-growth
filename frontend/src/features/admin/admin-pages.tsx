...
function InvitationRow({ item, selected, onToggleSelect, csrfToken, onNotice }: { item: Invitation; selected: boolean; onToggleSelect: () => void; csrfToken: string; onNotice: (message: string, error?: boolean) => void }) {
  ...
  return <TableRow><TableCell className="font-semibold"><label className="flex items-center gap-2"><input type="checkbox" checked={selected} onChange={onToggleSelect} aria-label={`选择邀请码 ${item.code}`} /><code className="font-mono text-sm">{item.code}</code></label></TableCell><TableCell><StatusBadge tone={item.status === "ACTIVE" ? "success" : "danger"}>{item.status === "ACTIVE" ? "启用" : "停用"}</StatusBadge></TableCell><TableCell className="text-slate-600">{item.used_count} 次{item.max_uses ? ` / ${item.max_uses} 次` : " / 无限次"}</TableCell><TableCell className="text-slate-600">{item.expires_at ? formatDate(item.expires_at) : "永不过期"}</TableCell><TableCell className="text-slate-600">{formatDate(item.created_at)}</TableCell><TableCell><div className="flex flex-wrap items-center gap-1.5"><Button variant="ghost" size="sm" onClick={() => toggleMutation.mutate()} loading={toggleMutation.isPending}>{item.status === "ACTIVE" ? "停用" : "启用"}</Button><Button variant="icon" size="sm" onClick={() => setDeleteOpen(true)} title="删除邀请码" aria-label={`删除邀请码 ${item.code}`}><Trash2 size={15} className="text-rose-700" /></Button></div><ConfirmDialog open={deleteOpen} onOpenChange={setDeleteOpen} title="删除邀请码" description={`确定永久删除邀请码“${item.code}”吗？已记录的使用记录也会一并清理，此操作无法撤销。`} confirmLabel="永久删除" loading={deleteMutation.isPending} onConfirm={() => deleteMutation.mutate()} /></TableCell></TableRow>;
}
