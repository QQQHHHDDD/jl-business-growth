...
export function FinancePage({ authResponse }: { authResponse: AuthResponse }) {
  ...
  const transactionMutation = useMutation({
    mutationFn: () => saveFinanceTransaction(authResponse.data.csrf_token, { occurred_on: transaction.occurred_on, type: transaction.type, category_id: transaction.category_id, amount: Number(transaction.amount), description: transaction.description.trim() || null, note: transaction.note.trim() || null, source: "MANUAL" }),
  ...
  const budgetMutation = useMutation({
    mutationFn: () => saveFinanceBudget(authResponse.data.csrf_token, { month: budget.month, amount: Number(budget.amount), category_id: null }),
    onSuccess: () => { setNotice("月度总预算已保存"); setError(""); invalidate("finance-budgets"); },
    onError: (value) => { setError(errorMessage(value)); setNotice(""); },
  });
  const snapshotMutation = useMutation({
    mutationFn: () => saveFinanceSnapshot(authResponse.data.csrf_token, { snapshot_date: snapshot.date, kind: snapshot.kind, amount: Number(snapshot.amount), note: snapshot.note.trim() || null }),
