...
export function ReviewsPage({ authResponse }: { authResponse: AuthResponse }) {
  ...
  return <div className="space-y-6">
    ...
    {reviewQuery.isPending ? <LoadingState label="正在加载复盘" /> : reviewQuery.isError ? <ErrorState message="复盘暂时无法加载" onRetry={() => void reviewQuery.refetch()} /> : <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_260px]">
      <main className="rounded-xl border border-slate-200 bg-white shadow-panel">
      ...
      <aside className="rounded-xl border border-slate-200 bg-white p-4 shadow-panel lg:sticky lg:top-24"><h2 className="font-bold text-slate-950">最近保存</h2><p className="mt-1 text-sm text-slate-500">仅显示真正保存过的{typeLabels[type]}复盘。</p>{historyQuery.isPending ? <div className="mt-4"><LoadingState label="正在读取记录" /></div> : historyQuery.isError ? <div className="mt-4"><ErrorState message="历史记录暂时无法加载" onRetry={() => void historyQuery.refetch()} /></div> : recents.length ? <div className="mt-4 space-y-1">{recents.map((item) => <button key={item.id ?? `${item.type}-${item.period_start}`} type="button" onClick={() => setPeriod(item.period_start)} className={`flex min-h-10 w-full items-center justify-between rounded-lg px-3 text-sm ${period === item.period_start ? "bg-teal-50 font-semibold text-teal-800" : "text-slate-600 hover:bg-slate-50"}`}><span>{type === "MONTHLY" ? item.period_start.slice(0, 7) : item.period_start}</span><ChevronRight size={15} /></button>)}</div> : <div className="mt-4"><EmptyState title="还没有已保存复盘" description="保存当前复盘后会显示在这里。" /></div>}</aside>
    </div>}
  </div>;
}
