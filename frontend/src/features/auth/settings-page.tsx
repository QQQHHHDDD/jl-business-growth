...
export function SettingsPage({ account, authResponse }: { account: Account; authResponse: AuthResponse }) {
  ...
  return <div className="space-y-6">
    ...
    {tab === "preferences" && <Panel title="业务时区" description="业务日期使用账号的 IANA 时区，不随服务器时间变化。"><form className="max-w-xl space-y-4" onSubmit={(event) => { event.preventDefault(); timezoneMutation.mutate(); }}><Input label="IANA 时区" value={timezone} onChange={(event) => setTimezone(event.target.value)} required maxLength={64} placeholder="Asia/Shanghai" /><p className="text-sm text-slate-500">系统中的业务日期边界、日历、工作量、营业额、复盘、统计和团队月末快照均以该时区为准。</p><Button type="submit" loading={timezoneMutation.isPending}><Globe2 size={15} />保存时区</Button></form></Panel>}
