...
import { Camera, Check, GitBranch, Plus, Trash2 } from "lucide-react";
...
function TeamGraph({ members, onSelect }: { members: GraphMember[]; onSelect?: (id: string) => void }) {
  ...
  return <div className="relative h-full min-w-[720px] overflow-auto bg-slate-50 bg-[radial-gradient(#e2e8f0_1px,transparent_1px)] [background-size:20px_20px]" role="img" aria-label="团队关系图"><div className="relative" style={{ width, height }}><svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox={`0 0 ${width} ${height}`} aria-hidden="true">{edges.map((edge) => { const source = nodeById.get(edge.source); const target = nodeById.get(edge.target); if (!source || !target) return null; return <path key={`${edge.source}-${edge.target}`} d={`M ${source.position.x + 124} ${source.position.y + 106} C ${source.position.x + 124} ${source.position.y + 140}, ${target.position.x + 124} ${target.position.y - 34}, ${target.position.x + 124} ${target.position.y + 20}`} fill="none" stroke="#0f766e" strokeWidth="2" />; })}</svg>{nodes.map((node) => <button key={node.id} type="button" onClick={() => onSelect?.(node.id)} className={`absolute w-[208px] rounded-xl border p-4 text-left shadow-float transition hover:-translate-y-0.5 hover:shadow-overlay focus:outline-none focus:ring-2 focus:ring-teal-500 ${node.data.isRoot ? "border-teal-700 bg-teal-700 text-white" : "border-teal-200 bg-white text-slate-900"}`} style={{ left: node.position.x + 20, top: node.position.y + 20 }}><span className={`absolute bottom-0 left-0 top-0 w-[3px] rounded-l-xl ${node.data.isRoot ? "bg-teal-300" : "bg-teal-500"}`} aria-hidden="true" /><span className="block font-bold">{node.data.name}</span><span className={`mt-1 block text-xs ${node.data.isRoot ? "text-teal-50" : "text-slate-500"}`}>{node.data.rank || "未设置级别"} · {node.data.city || "未设置城市"}</span><span className={`mt-3 inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${node.data.status === "ACTIVE" ? (node.data.isRoot ? "bg-white/20 text-white" : "bg-emerald-50 text-emerald-700") : "bg-slate-100 text-slate-600"}`}>{node.data.status === "ACTIVE" ? "启用" : "停用"}</span></button>)}</div></div>;
}
...
export function TeamPage({ authResponse }: { authResponse: AuthResponse }) {
  ...
  const filteredMembers = members.filter((member) => [member.name, member.member_code, member.rank, member.city].some((value) => value?.toLowerCase().includes(search.toLowerCase())));
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><Tabs value={view} onValueChange={(value) => setView(value as TeamView)}><TabsList aria-label="团队视图"><TabsTrigger value="graph">关系图</TabsTrigger><TabsTrigger value="list">成员列表</TabsTrigger><TabsTrigger value="snapshots">历史快照</TabsTrigger></TabsList></Tabs>{view !== "snapshots" && <div className="w-full sm:max-w-xs"><Input label="搜索成员" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="姓名、编码、级别或城市" /></div>}</div>
    {view === "graph" && <Panel title="团队关系图" description="拖动滚动区域查看完整结构，点击成员打开详情。"><div className="h-[min(68vh,720px)] min-h-[500px] overflow-auto rounded-xl border border-slate-200"><TeamGraph members={filteredMembers} onSelect={(id) => { const member = members.find((item) => item.id === id); if (member) openDetail(member); }} /></div></Panel>}
