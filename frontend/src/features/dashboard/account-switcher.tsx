...
import { addAccount, ApiError, switchAccount, unlinkAccount, type AuthResponse } from "@/api/client";
...
import { errorMessage, roleLabel } from "@/lib/utils";
...
export function AccountSwitcher({ authResponse }: { authResponse: AuthResponse }) {
  ...
  const unlinkMutation = useMutation({
    ...
    onSuccess: (_, accountId) => { removeLinkedAccount(accountId); setConfirmId(null); setNotice("浏览器账号关联已解除。"); setError(""); },
    onError: (value, accountId) => {
      if (accountId && value instanceof ApiError && value.status === 404) {
        removeLinkedAccount(accountId);
        setConfirmId(null);
        setNotice("该账号关联已失效，已从浏览器列表移除。");
        setError("");
  ...
  return <Panel title="浏览器账号" description="已验证的账号可以在此浏览器快速切换，管理员和超级管理员会明确标注。切换时会清理旧账号的业务缓存。" action={<Button variant="secondary" size="sm" onClick={() => { setError(""); setOpen(true); }}><Plus size={15} />添加账号</Button>}>
    ...
    <div className="grid gap-2 sm:grid-cols-2">{accounts.map((item) => <div key={item.id} className={`flex items-center justify-between rounded-md border px-4 py-3 ${item.active ? "border-teal-300 bg-teal-50" : "border-slate-200"}`}><div><div className="flex items-center gap-2"><p className="font-semibold text-slate-900">{item.username}</p><span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${item.role === "SUPER_ADMIN" ? "bg-violet-100 text-violet-800" : item.role === "ADMIN" ? "bg-sky-100 text-sky-800" : "bg-slate-100 text-slate-600"}`}>{roleLabel(item.role)}</span></div><p className="mt-1 text-xs text-slate-500">{item.active ? "当前使用" : "已验证"}</p></div>{!item.active && <div className="flex items-center gap-2"><Button variant="ghost" size="sm" onClick={() => switchMutation.mutate(item.id)} loading={switchMutation.isPending && switchMutation.variables === item.id}>切换</Button><Button variant="icon" size="sm" onClick={() => setConfirmId(item.id)} aria-label={`解除浏览器关联 ${item.username}`} title="解除浏览器关联"><UserMinus size={16} className="text-rose-700" /></Button></div>}</div>)}</div>
    <Dialog open={open} onOpenChange={setOpen}><DialogContent><DialogHeader><DialogTitle>添加浏览器账号</DialogTitle><DialogDescription>验证账号后，它会加入当前浏览器的快捷切换列表，并保留对应的角色标识。</DialogDescription></DialogHeader><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); addMutation.mutate(); }}><Input label="账号" value={username} onChange={(event) => setUsername(event.target.value)} required maxLength={32} autoComplete="username" /><Input label="密码" type="password" value={password} onChange={(event) => setPassword(event.target.value)} required minLength={10} maxLength={128} autoComplete="current-password" />{error && <p role="alert" className="text-sm text-rose-700">{error}</p>}<div className="flex justify-end gap-3"><Button variant="secondary" size="sm" type="button" onClick={() => setOpen(false)}>取消</Button><Button size="sm" type="submit" loading={addMutation.isPending}><KeyRound size={15} />验证并加入</Button></div></form></DialogContent></Dialog>
