...
function DashboardContent({ data }: { data: DashboardResponse }) {
  ...
  return <><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Panel title="今日工作量"><p className="text-2xl font-bold text-slate-950">{worklogTotal(today)}</p><p className="mt-1 text-sm text-slate-500">个行动记录</p></Panel><Panel title="本周工作量"><p className="text-2xl font-bold text-slate-950">{worklogTotal(week)}</p><p className="mt-1 text-sm text-slate-500">个行动记录</p></Panel><Panel title="本月 PV"><p className="text-2xl font-bold text-slate-950">{month.turnover.pv.toLocaleString("zh-CN")}</p><p className="mt-1 text-sm text-slate-500">净营业额 {money(month.turnover.net_amount)}</p></Panel><Panel title="梦想数量"><p className="text-2xl font-bold text-slate-950">{data.data.dreams_count}</p><p className="mt-1 text-sm text-slate-500">已保存的方向</p></Panel></div><div className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]"><Panel title="营业额节奏" description="营业额来自每日营业额唯一事实源。"><div className="grid gap-4 sm:grid-cols-3"><PeriodValue label="今日" pv={today.turnover.pv} amount={today.turnover.net_amount} /><PeriodValue label="本周" pv={week.turnover.pv} amount={week.turnover.net_amount} /><PeriodValue label="本月" pv={month.turnover.pv} amount={month.turnover.net_amount} /></div></Panel><Panel title="当前目标" description="进度由目标日期范围内的真实记录实时计算。">{data.data.active_goals.length ? <div className="space-y-4">{data.data.active_goals.map((goal) => <GoalProgress key={goal.id} goal={goal} />)}</div> : <EmptyState title="还没有进行中的目标" description="建立一个目标后，工作台会显示真实进度。" action={<Button asChild variant="secondary" size="sm"><Link to="/app/goals"><GoalIcon size={15} />建立目标</Link></Button>} />}</Panel></div></>;
}

function PeriodValue({ label, pv, amount }: { label: string; pv: number; amount: number }) {
  return <div className="rounded-md bg-slate-50 p-4"><p className="text-xs font-semibold text-slate-500">{label}</p><p className="mt-2 text-lg font-bold text-slate-950">{pv.toLocaleString("zh-CN")} PV</p><p className="mt-1 text-xs text-slate-500">{money(amount)}</p></div>;
}
