...
export function SearchPage({ authResponse }: { authResponse: AuthResponse }) {
  ...
  return <div className="search-page space-y-6"><PageHeader eyebrow="统一检索" title="全局搜索" description="搜索目标、日历、团队和学习内容（含标签）；附件正文不参与搜索。" /><Panel className="search-filter-panel"><form onSubmit={(event) => { event.preventDefault(); runSearch(); }}><div className="flex flex-col gap-3 sm:flex-row sm:items-end"><Input label="搜索关键词" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="输入关键词" className="flex-1" /><Button type="submit" loading={search.isPending} disabled={!query.trim() || selectedModules.length === 0}><SearchIcon size={16} />搜索</Button></div><fieldset className="mt-5"><legend className="text-sm font-semibold text-ink-muted">搜索范围</legend><div className="mt-2 flex flex-wrap gap-2">{modules.map(([value, label]) => <label key={value} className={`search-scope inline-flex cursor-pointer items-center gap-2 rounded-control border px-3 py-2 text-sm font-semibold ${selectedModules.includes(value) ? "search-scope-active border-brand-300 bg-brand-50 text-brand-900" : "border-outline/70 bg-surface text-ink-muted"}`}><input type="checkbox" checked={selectedModules.includes(value)} onChange={() => toggleModule(value)} />{label}</label>)}</div></fieldset></form>{recentSearches.length > 0 && <div className="mt-5 border-t border-outline/45 pt-4"><div className="flex items-center justify-between"><p className="text-sm font-semibold text-ink-muted">最近搜索</p><Button variant="ghost" size="sm" onClick={() => { setRecentSearches([]); localStorage.removeItem(storageKey); }}><X size={14} />清空</Button></div><div className="mt-2 flex flex-wrap gap-2">{recentSearches.map((item) => <button key={item} type="button" className="search-history-chip rounded-full border border-outline/55 bg-surface-soft px-3 py-1.5 text-sm font-semibold text-ink-muted hover:border-brand-200 hover:bg-brand-50 hover:text-brand-800" onClick={() => runSearch(item)}>{item}</button>)}</div></div>}</Panel>{search.isError && <ErrorState message={errorMessage(search.error)} onRetry={() => runSearch()} />}{search.data && <Panel className="search-results-panel" title="搜索结果" description={`共找到 ${search.data.meta.total} 条匹配记录。`}>{search.data.data.items.length === 0 ? <EmptyState title="没有匹配记录" description="尝试更短的关键词，或扩大搜索范围。" /> : <div className="space-y-6">{groupedResults.map(([module, items]) => <section key={module} aria-labelledby={`search-group-${module}`}><h2 id={`search-group-${module}`} className="mb-2 flex items-center gap-2 text-sm font-bold text-ink-muted">{labels[module] ?? module}<span className="rounded-full bg-brand-50 px-2 py-0.5 text-xs font-semibold text-brand-700">{items.length} 条</span></h2><div className="search-result-group divide-y divide-outline/40 overflow-hidden rounded-card border border-outline/65 bg-surface">{items.map((item) => <Link key={`${item.module}-${item.id}`} to={resultRoute(item)} className="group flex min-h-20 items-center justify-between gap-4 px-4 py-3 hover:bg-brand-50/55"><span className="min-w-0"><strong className="block truncate text-ink group-hover:text-brand-800">{item.title}</strong><span className="mt-1 block line-clamp-2 text-sm leading-6 text-ink-faint">{item.snippet}</span></span><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-surface-soft text-ink-faint group-hover:bg-brand-100 group-hover:text-brand-800"><ArrowRight size={16} /></span></Link>)}</div></section>)}</div>}</Panel>}</div>;
}
