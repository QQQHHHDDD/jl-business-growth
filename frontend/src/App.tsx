...
function ProtectedRoute({ meQuery, children }: { meQuery: MeQuery; children?: ReactNode }) {
  ...
  if (meQuery.isError) return <ErrorState message="无法确认当前登录状态" onRetry={() => void meQuery.refetch()} />;
  if (!meQuery.data) return <Navigate to="/login" state={{ from: `${location.pathname}${location.search}` }} replace />;
  return children ?? <Outlet />;
}

function PublicRoute({ meQuery, mode, health }: { meQuery: MeQuery; mode: "login" | "register"; health: HealthQuery }) {
  if (meQuery.isPending) return <LoadingState label="正在准备登录" />;
  if (meQuery.data) return <Navigate to={roleHome(meQuery.data.data.account.role)} replace />;
  return <AuthPage key={mode} mode={mode} health={health} />;
}

function RoleRoute({ meQuery, roles, children }: { meQuery: MeQuery; roles: string[]; children?: ReactNode }) {
  if (!meQuery.data) return null;
  const account = meQuery.data.data.account;
  if (!roles.includes(account.role)) return <Navigate to={roleHome(account.role)} replace />;
  return children ?? <Outlet />;
}

function ShellRoute({ meQuery, health, admin, onLogout, loggingOut, children }: { meQuery: MeQuery; health: HealthQuery; admin: boolean; onLogout: () => void; loggingOut: boolean; children?: ReactNode }) {
  if (!meQuery.data) return null;
  return <AppShell account={meQuery.data.data.account} health={health} admin={admin} onLogout={onLogout} loggingOut={loggingOut}>{children ?? <Outlet />}</AppShell>;
}

function HomeRedirect({ meQuery }: { meQuery: MeQuery }) {
  if (meQuery.isPending) return <div className="min-h-screen bg-slate-100 p-8"><h1 className="sr-only">JL团队生意成长管理系统</h1><LoadingState label="正在准备工作台" /></div>;
  return <Navigate to={meQuery.data ? roleHome(meQuery.data.data.account.role) : "/login"} replace />;
}
...
function SettingsRoute({ meQuery }: { meQuery: MeQuery }) {
  const authResponse = meQuery.data;
  if (!authResponse) return null;
  return <SettingsPage account={authResponse.data.account} authResponse={authResponse} />;
}
...
function AdminInvitationsRoute({ meQuery }: { meQuery: MeQuery }) {
  return meQuery.data ? <AdminInvitationsPage authResponse={meQuery.data} /> : null;
}
...
function AppRoutes({ meQuery, health, onLogout, loggingOut }: { meQuery: MeQuery; health: HealthQuery; onLogout: () => void; loggingOut: boolean }) {
  return <Routes>
    <Route path="/" element={<HomeRedirect meQuery={meQuery} />} />
    <Route path="/login" element={<PublicRoute meQuery={meQuery} mode="login" health={health} />} />
    <Route path="/register" element={<PublicRoute meQuery={meQuery} mode="register" health={health} />} />
    <Route element={<ProtectedRoute meQuery={meQuery} />}>
      ...
      <Route element={<RoleRoute meQuery={meQuery} roles={["USER", "ADMIN", "SUPER_ADMIN"]} />}>
        <Route element={<ShellRoute meQuery={meQuery} health={health} admin={meQuery.data?.data.account.role !== "USER"} onLogout={onLogout} loggingOut={loggingOut} />}>
          <Route path="/app/settings" element={<SettingsRoute meQuery={meQuery} />} />
        </Route>
      </Route>
      <Route element={<RoleRoute meQuery={meQuery} roles={["ADMIN", "SUPER_ADMIN"]} />}>
        <Route element={<ShellRoute meQuery={meQuery} health={health} admin onLogout={onLogout} loggingOut={loggingOut} />}>
          <Route path="/admin" element={<AdminHomeRoute meQuery={meQuery} />} />
          <Route path="/admin/users" element={<AdminUsersRoute meQuery={meQuery} />} />
          <Route path="/admin/invitations" element={<AdminInvitationsRoute meQuery={meQuery} />} />
          <Route element={<RoleRoute meQuery={meQuery} roles={["SUPER_ADMIN"]} />}><Route path="/admin/admins" element={<AdminAdminsRoute meQuery={meQuery} />} /></Route>
        </Route>
      </Route>
      <Route path="*" element={<UnknownProtectedRedirect meQuery={meQuery} />} />
    </Route>
...
function AppContent() {
  ...
  const healthQuery = useQuery({ queryKey: ["health", "live"], queryFn: getLiveHealth });
  ...
  useEffect(() => {
    const account = meQuery.data?.data.account;
    document.title = account ? `${account.username} · JL团队生意成长管理系统` : "JL团队生意成长管理系统";
  }, [meQuery.data]);

  return <AppRoutes meQuery={meQuery} health={healthQuery} onLogout={() => logoutMutation.mutate()} loggingOut={logoutMutation.isPending} />;
}
