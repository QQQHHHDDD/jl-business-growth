...
export function PromptDialog({ open, onOpenChange, title, description, label, value, onValueChange, confirmLabel = "确认", loading = false, error, onConfirm }: { open: boolean; onOpenChange: (open: boolean) => void; title: string; description: string; label: string; value: string; onValueChange: (value: string) => void; confirmLabel?: string; loading?: boolean; error?: string; onConfirm: () => void }) {
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>{title}</DialogTitle><DialogDescription>{description}</DialogDescription></DialogHeader><Input label={label} type="password" value={value} onChange={(event) => onValueChange(event.target.value)} minLength={10} maxLength={128} placeholder="留空由系统生成" autoComplete="new-password" error={error} /><div className="mt-6 flex justify-end gap-3"><Button variant="secondary" size="sm" onClick={() => onOpenChange(false)} disabled={loading}>取消</Button><Button size="sm" onClick={onConfirm} loading={loading}> {confirmLabel}</Button></div></DialogContent></Dialog>;
}
