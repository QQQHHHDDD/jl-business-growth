  ...
  return (
    <div className="space-y-6">
      ...
      <div
        role="status"
        aria-live="polite"
        className="min-h-[420px] rounded-panel border border-outline/80 bg-surface p-6 shadow-panel"
      >
        <span className="sr-only">{label}...</span>
...
export function EmptyState({ title = "暂无内容", description, action, compact = false, className }: { title?: string; description?: string; action?: ReactNode; compact?: boolean; className?: string }) {
  return <div className={cn("flex flex-col items-center justify-center gap-2 rounded-panel border border-dashed border-outline/80 bg-surface-soft p-8 text-center shadow-hairline", compact ? "min-h-40" : "min-h-[220px]", className)}><span className="grid h-11 w-11 place-items-center rounded-full bg-brand-100 text-brand-700 shadow-hairline"><Inbox size={22} aria-hidden="true" /></span><h3 className="text-sm font-bold text-ink">{title}</h3>{description && <p className="max-w-md text-sm text-ink-muted">{description}</p>}{action}</div>;
}
