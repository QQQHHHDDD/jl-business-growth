...
export type CommunicationScriptType = components["schemas"]["CommunicationScriptType"];
export type CommunicationScriptTypeRequest = components["schemas"]["CommunicationScriptTypeRequest"];
...
export function listCommunicationScriptTypes(): Promise<components["schemas"]["CommunicationScriptTypeListResponse"]> { return request("/api/communication/script-types"); }
export function saveCommunicationScriptType(csrfToken: string, input: CommunicationScriptTypeRequest, id?: string): Promise<components["schemas"]["CommunicationScriptTypeResponse"]> { return request(id ? `/api/communication/script-types/${encodeURIComponent(id)}` : "/api/communication/script-types", withCsrf(csrfToken, input as unknown as JSONValue, id ? "PUT" : "POST")); }
export function deleteCommunicationScriptType(csrfToken: string, id: string): Promise<void> { return request(`/api/communication/script-types/${encodeURIComponent(id)}`, withCsrf(csrfToken, undefined, "DELETE")); }
export function listCommunicationScripts(filters: { page?: number; pageSize?: number; q?: string; scriptType?: string; favorite?: boolean } = {}): Promise<components["schemas"]["CommunicationScriptListResponse"]> { const params = new URLSearchParams(); if (filters.page) params.set("page", String(filters.page)); if (filters.pageSize) params.set("page_size", String(filters.pageSize)); if (filters.q) params.set("q", filters.q); if (filters.scriptType) params.set("script_type", filters.scriptType); if (filters.favorite) params.set("favorite", "true"); return request(`/api/communication/scripts${params.size ? `?${params}` : ""}`); }
