...
- migration 默认 append-only，只能通过新增 migration 演进 schema，并同时考虑 fresh database 和从已发布版本升级的路径。当前终点为 `00016_communication_script_types.sql`；除已记录的 pgcrypto 兼容例外外，不得修改已发布 migration。
