...
grep -Eq 'pg_dump.*DATABASE_URL' "${backup_script}" && exit 1
root="$(mktemp -d)"
...
grep -Fq "${database_url}" "${root}/pg_dump.argv" && exit 1
grep -Fq 'VERY_SECRET_TEST_PASSWORD' "${root}/pg_dump.argv" && exit 1
